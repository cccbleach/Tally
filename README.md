...
原生 SwiftUI iOS 客户端 + 自建 Node.js 后端的记账应用。专注记账本身：收支流水、分类、统计图表、多账户、周期账单与共享账本，数据通过自建后端实现多设备同步。
...
- 💳 **多账户**：现金/银行卡/电子钱包/其他，实时余额，账户间转账
- 🔁 **周期账单**：房租、订阅等定期账单自动生成流水（每日调度，幂等）
- 📥 **账单导入**：支持导入微信/支付宝/银行导出的账单文件（txt/csv/xlsx/pdf），服务端解析并跨来源去重入库（重复导入不重复入账）
- 🔍 **截图识别**：微信/支付宝支付成功页截图在本机 Vision OCR 识别后走同一条导入管道（截图不上传，识别结果先预览再入账，同一截图重复导入自动去重）
- ⚡ **快捷指令记账**：App Intents「记一笔」（金额/类型/备注参数），支持 Siri、轻点背面、锁屏与操作按钮调用，无需打开 App；断网时自动落离线队列
- 🟦 **桌面小组件**：一键记账小组件，点按经 tally://add 深链直达记账表单
- 🔒 **生物锁**：Face ID / 触控 ID 上锁，App 切后台即锁定（可选开关）
- 📴 **离线记账**：断网记一笔先入本地队列（按用户+账本隔离，快捷指令同一条队列），联网/回前台自动重放；重放携带客户端幂等键，服务端保证不重复入账
- ⚖️ **乐观锁全覆盖**：流水/周期账单/账户/分类的 PATCH 均支持 expectedUpdatedAt，家庭共享场景并发修改返回 409 而非静默覆盖
- 📤 **流水导出**：设置页一键导出当前账本全部流水为 CSV（UTF-8 BOM + 币种列 + OWASP 公式注入防护），可保存/分享备份
...
cd backend
pnpm typecheck
pnpm test        # 165 个测试：手机号+短信验证码认证/强制唯一昵称/共享账本/账户/分类/流水/统计/周期账单/去重/乐观锁/流水幂等键
                 # （含幂等、OpenAPI 契约、时区、转账检索、账本隔离、分层、
                 #  迁移校验（28 个迁移，含 0025-0028 的下线与折算回归）、乐观锁、并发回归、故障注入、
                 #  限流与 TRUST_PROXY、AUTH_MODE/限流生产互锁、短信节流、账单币种护栏（仅人民币）、
                 #  外币金额折算入备注、服务端登出与契约集合校验、生产模式安全策略、账单导入去重等回归，全部 0 fail）
```
