...
- ⚖️ **乐观锁全覆盖**：流水/预算/周期账单/账户/分类的 PATCH 均支持 expectedUpdatedAt，家庭共享场景并发修改返回 409 而非静默覆盖
- 📤 **流水导出**：设置页一键导出当前账本全部流水为 CSV（UTF-8 BOM + 币种列 + OWASP 公式注入防护），可保存/分享备份
- 💱 **多币种**：账户/流水支持各自币种（录入跟随账户币种，跨币种写入被服务端护栏拦截）；统计/预算/负债由服务端折算到账本本位币展示，客户端按币种精度格式化（金额全程整数最小单位，无浮点运算）
...
cd backend
pnpm typecheck
pnpm test        # 209 个测试：手机号+短信验证码认证/强制唯一昵称/共享账本/账户/分类/流水/统计/预算/周期账单/贷款/去重/乐观锁/流水幂等键/汇率拉取
                 # （含幂等、OpenAPI 契约、时区、转账检索、汇率换算、负债语义、账本隔离、分层、
                 #  迁移校验（24 个迁移）、乐观锁、并发回归、故障注入、限流与 TRUST_PROXY、
