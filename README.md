...
原生 SwiftUI iOS 客户端 + 自建 Node.js 后端的记账应用。专注记账本身：收支流水、分类、统计图表、周期账单与共享账本，数据通过自建后端实现多设备同步。
...
- ⚖️ **乐观锁全覆盖**：流水/周期账单/分类的 PATCH 均支持 expectedUpdatedAt，家庭共享场景并发修改返回 409 而非静默覆盖
...
cd backend
pnpm typecheck
pnpm test        # 166 个测试：手机号+短信验证码认证/强制唯一昵称/共享账本/分类/流水/统计/周期账单/去重/乐观锁/流水幂等键/账单关键词自动分类
                 # （含幂等、OpenAPI 契约、时区、账本隔离、分层、
                 #  迁移校验（29 个迁移，含 0025-0029 的下线与折算回归）、乐观锁、并发回归、故障注入、
